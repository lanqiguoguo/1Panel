...
const message = {
    commons: {
        ...
        login: {
            ...
            encryptErr: 'Не удалось зашифровать пароль, обновите страницу и попробуйте снова!',
    ...
    setting: {
        ...
        proxyHelper1: 'Синхронизация индекса магазина приложений, загрузка значков/композиций и установочных пакетов',
        proxyHelper2: 'Обновление системы (проверка версии, примечания к обновлению и загрузка пакета обновления)',
        proxyHelper4:
            'При включении «Синхронизировать с Docker» прокси будет записан в конфигурацию демона Docker (/etc/docker/daemon.json), а Docker будет перезапущен, чтобы запросы (например, загрузка образов) шли через прокси; во время перезапуска контейнеры кратковременно останавливаются',
        proxyHelper3: 'Заявка на сертификат (регистрация аккаунта ACME/заказы сертификатов и получение ZeroSSL EAB)',
        proxyType: 'Тип прокси',
        proxyUrl: 'Адрес прокси',
        proxyPort: 'Порт прокси',
        proxyPasswdKeep: 'Запомнить пароль',
        proxyDocker: 'Синхронизировать с Docker',
        proxyDockerHelper:
        ...
        proxyDockerSyncConfirm:
            'После сохранения прокси будет записан в конфигурацию демона Docker (/etc/docker/daemon.json), а Docker будет перезапущен. Работающие контейнеры кратковременно остановятся во время перезапуска (не затрагиваются, если live-restore включен в настройках контейнеров). Продолжить?',
