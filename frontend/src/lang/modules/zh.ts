...
const message = {
    ...
    setting: {
        ...
        proxyHelper1: '应用商店的索引同步、图标/编排下载与安装包下载',
        proxyHelper2: '系统版本升级（版本检查、更新说明与升级包下载）',
        proxyHelper3: '申请证书（ACME 账户注册/证书订单与 ZeroSSL EAB 获取）',
        proxyType: '代理类型',
        proxyDisable: '不启用',
        proxyOptional: '选填',
        proxyUser: '用户名',
        ...
        proxyHelper4:
            '勾选"同步到 Docker"后，代理将写入 Docker daemon 配置（/etc/docker/daemon.json）并重启 Docker，使镜像拉取等请求生效；重启期间容器会短暂停止',
        proxyDockerSyncConfirm:
            '保存后代理将写入 Docker daemon 配置（/etc/docker/daemon.json）并重启 Docker，重启期间运行中的容器会短暂停止（若已在容器设置中开启 live-restore 则不受影响）。是否继续？',
        apiInterface: 'API 接口',
