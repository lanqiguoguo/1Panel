...
const searchLogs = async () => {
    ...
    terminalSocket.value = new WebSocket(
        `${protocol}://${host}/api/v1/containers/search/log?container=${encodeURIComponent(
            logSearch.containerID,
        )}&since=${logSearch.mode}&tail=${logSearch.tail}&follow=${logSearch.isWatch}`,
    );
