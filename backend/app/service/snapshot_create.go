...
func handleSnapTar(sourceDir, targetDir, name, exclusionRules string, secret string) error {
	...
	if len(secret) != 0 {
		// Same mechanism as handleTar in cronjob_helper.go: the secret travels
		// on inherited fd 3 instead of the command line, so it never appears
		// in the bash -c argv (or any /proc/<pid>/cmdline) and the debug log
		// needs no masking.
		...
		stdout, err = cmd.ExecWithTimeOutExtraFiles(commands, 30*time.Minute, []*os.File{secretReader})
	} else {
